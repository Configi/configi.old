#!bin/lua
local factid = require"factid"
local lib = require"lib"
local os, file, path, table, exec = lib.os, lib.file, lib.path, lib.table, lib.exec
local cmd = lib.exec.cmd
local crc = require"crc32".crc32_string
local string = string
local stat, pwd, grp = require"posix.sys.stat", require"posix.pwd", require"posix.grp"
local ct = require"cwtest"
local T, N=ct.new(), nil
local osfamily = factid.osfamily()
local testdir = "test/tmp/"
local cfg = cmd["bin/cfg-agent.lua"]

if not os.is_dir(testdir) then
  stat.mkdir(testdir)
end

T:start"Lua tests"
  do
    local Lua = dofile("test/lua.lua")
    T:eq(Lua.sequence(), "12345")
    T:eq(Lua.ipairsnil(), "123")
    T:eq(Lua.forloopnil(), "1235")
    T:eq(Lua.nextsequence(), "1235")
    T:eq(Lua.multiplereturn(), "145")
    T:eq(Lua.updatetable1(), "12345")
    T:eq(Lua.updatetable2(), "12345")
    T:eq(Lua.mixedtable1(), "012")
    T:eq(Lua.mixedtable2(), "123")
  end
T:done(N)

T:start"embedded policy test/core-embedded"
  do
    local e = function(policy)
      local _, out = cfg{ "-m", "-e", policy }
      T:neq(string.find(out.stderr[1], "%C+%c%C+%c%C+%c#test includes"), nil)
      T:neq(string.find(out.stderr[2], "%C+%c%C+%c%C+%c#test includes"), nil)
      T:neq(string.find(out.stderr[3], "%C+%c%C+%c%C+"), nil)
      T:neq(string.find(out.stderr[4], "%C+%c%C+%c%C+"), nil)
      T:neq(string.find(out.stderr[5], "%C+%c%C+%c%C+"), nil)
      T:neq(string.find(out.stderr[6], "%C+%c%C+%c%C+"), nil)
      T:neq(string.find(out.stderr[7], "%C+%c%C+%c%C+%c#test handlers"), nil)
      T:neq(string.find(out.stderr[8], "%C+%c%C+%c%C+%c#test handlers"), nil)
      T:yes(stat.stat"test/tmp/core-embedded-structure")
      T:yes(stat.stat"test/tmp/core-embedded-handlers")
      T:yes(stat.stat"test/tmp/core-embedded.txt")
      local stat2 = stat.stat(testdir .. "core-embedded.txt")
      T:eq(string.format("%o", stat2.st_mode), "100777")
      os.remove(testdir.."core-embedded-structure")
      os.remove(testdir.."core-embedded-handlers")
      os.remove(testdir.."core-embedded.txt")
    end
    e"init.lua"
  end
T:done(N)

T:start"roles test/core-roles"
  do
    local c
    c = function(p)
      local f
      f = {
        "core-role",
        "core-role-attrib",
        "core-role-attrib-one",
        "core-role-handler",
        "core-role-handler-one",
        "core-role-modules",
        "core-role-modules-one",
        "core-role-one",
        "core-role-policy",
        "core-role-policy-one",
        "core-role-top"
      }
      cfg{"-f", p}
      for _, t in ipairs(f) do
        T:yes(stat.stat(testdir..t))
        os.remove(testdir..t)
      end
    end
    c"test/core-roles/test.lua"
  end
T:done(N)

T:start"directory structure test/core-structure"
  do
    local st = function(policy)
      cfg{"-f", policy}
      T:yes(stat.stat"test/tmp/core-structure-attributes")
      T:yes(stat.stat"test/tmp/core-structure-policies")
      T:yes(stat.stat"test/tmp/core-structure-handlers")
      T:yes(stat.stat"test/tmp/copied-under-files")
      T:yes(stat.stat"test/tmp/template_render_test.txt")
      local t = file.read_to_string("test/tmp/template_render_test.txt")
      local m1, m2 = string.match(t, "^(Joe spends 6)\n(\n)")
      T:eq(m1, "Joe spends 6")
      T:eq(m2, "\n")
      T:yes(stat.stat"test/tmp/core-structure-role-copy")
      T:yes(stat.stat"test/tmp/core-structure-override-role-copy")
      t = file.read_to_string("test/tmp/core-structure-override-role-copy")
      m1 = string.match(t, "^(1)")
      T:eq(m1, "1")
      T:yes(stat.stat"test/tmp/core-structure-role-template")
      t = file.read_to_string("test/tmp/core-structure-role-template")
      m1 = string.match(t, "^(Joe spends 6)")
      T:eq(m1, "Joe spends 6")
      T:yes(stat.stat"test/tmp/core-structure-override-role-template")
      t = file.read_to_string("test/tmp/core-structure-override-role-template")
      m1 = string.match(t, "^(1 Joe spends 6)")
      T:eq(m1, "1 Joe spends 6")
      cmd.rm{"test/tmp/core-structure-attributes"}
      cmd.rm{"test/tmp/core-structure-policies"}
      cmd.rm{"test/tmp/core-structure-handlers"}
      cmd.rm{"test/tmp/copied-under-files"}
      cmd.rm{"test/tmp/template_render_test.txt"}
      cmd.rm{"test/tmp/core-structure-override-role-template"}
      cmd.rm{"test/tmp/core-structure-override-role-copy"}
      cmd.rm{"test/tmp/core-structure-role-template"}
      cmd.rm{"test/tmp/core-structure-role-copy"}
    end
    st("test/core-structure/test.lua")
  end
T:done(N)

T:start"debug test/core-debug.lua"
  do
    local debug = function(policy)
      local _, out = cfg{ "-vf", policy }
      out = table.concat(out.stderr, "\n")
      T:eq(string.find(out, "TESTDEBUG", 1, true), 70)
    end
    debug"test/core-debug.lua"
  end
T:done(N)

T:start"log test/core-log.lua"
  do
    local log = function(policy)
      local log = "test/tmp/_test_configi.log"
      cfg{ "-ltest/tmp/_test_configi.log", "-f", policy }
      T:yes(stat.stat(log))
      os.remove(log)
    end
    log"test/core-log.lua"
  end
T:done(N)

T:start"fact test/core-fact.lua"
  do
    local fact = function(policy)
      local code, out = cfg{ "-f", policy }
      T:eq(code, 0)
      T:eq(cmd.rm{"-f", testdir .. "i-deadbeef"}, 0)
    end
    fact"test/core-fact.lua"
  end
T:done(N)

T:start"test test/core-test.lua"
  do
    local test = function(policy)
      local tempname = testdir .. "core-test.txt"
      file.write_all(tempname, "test")
      cfg{ "-tf", policy}
      T:eq(file.read_to_string(tempname), "test")
      os.remove(tempname)
    end
    test"test/core-test.lua"
  end
T:done(N)

T:start"module test/core-module.lua"
  do
    local module = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    module"test/core-module.lua"
  end
T:done(N)

T:start"handler,notify test/core-handler.lua"
  do
    local h = function(handler,handler2)
      cfg{ "-f", handler }
      local xfile, file = "test/tmp/core-handler2-xfile", "test/tmp/core-handler2-file"
      cfg{ "-g", "testhandle", "-f",  handler2 }
      T:no(stat.stat(xfile))
      T:yes(stat.stat(file))
      os.remove(file)
    end
    h("test/core-handler.lua", "test/core-handler2.lua")
  end
T:done(N)

T:start"context test/core-context.lua"
  do
    cfg{ "-f", "test/core-context.lua"}
    T:no(stat.stat("test/tmp/core-context"))
  end
T:done(N)

T:start"comment test/core-comment.lua"
  do
    local comment = function(policy)
      local _, out = cfg{ "-v", "-f", policy }
      out = table.concat(out.stderr, "\n")
      T:eq(string.find(out, "TEST COMMENT", 1, true), 51)
    end
    comment"test/core-comment.lua"
  end
T:done(N)


T:start"require test/core-require.lua"
  do
    local require = function(policy)
      local _, out = cfg{ "-m", "-f", policy }
      T:neq(string.find(out.stderr[1], "%C+%c%C+%c%C+%c#1st"), nil)
      T:neq(string.find(out.stderr[2], "%C+%c%C+%c%C+%c#2nd"), nil)
      T:neq(string.find(out.stderr[3], "%C+%c%C+%c%C+%c#2nd"), nil)
      T:neq(string.find(out.stderr[4], "%C+%c%C+%c%C+%c#3rd"), nil)
      T:neq(string.find(out.stderr[5], "%C+%c%C+%c%C+%c#3rd"), nil)
      T:neq(string.find(out.stderr[6], "%C+%c%C+%c%C+%c#4th"), nil)
      T:neq(string.find(out.stderr[7], "%C+%c%C+%c%C+%c#4th"), nil)
      T:neq(string.find(out.stderr[8], "%C+%c%C+%c%C+%c#nodeps"), nil)
      T:neq(string.find(out.stderr[9], "%C+%c%C+%c%C+%c#nodeps"), nil)
      T:neq(string.find(out.stderr[10], "%C+%c%C+%c%C+%c#delete%-nodeps"), nil)
      T:neq(string.find(out.stderr[11], "%C+%c%C+%c%C+%c#delete%-nodeps"), nil)
      T:neq(string.find(out.stderr[12], "%C+%c%C+%c%C+%c#last"), nil)
      T:neq(string.find(out.stderr[13], "%C+%c%C+%c%C+%c#last"), nil)
      T:neq(string.find(out.stderr[14], "%C+%c%C+%c%C+%c#delete%-last"), nil)
      T:neq(string.find(out.stderr[15], "%C+%c%C+%c%C+%c#delete%-last"), nil)
      cmd.rm { "-f", testdir .. "core-require-first" }
      cmd.rm { "-f", testdir .. "core-require-another" }
      cmd.rm { "-f", testdir .. "core-require" }
    end
    local require_nodeps = function(policy)
      local _, out = cfg{ "-m", "-f", policy }
      T:neq(string.find(out.stderr[1], "%C+%c%C+%c%C+%c#nodeps"), nil)
      T:neq(string.find(out.stderr[2], "%C+%c%C+%c%C+%c#nodeps"), nil)
      T:neq(string.find(out.stderr[3], "%C+%c%C+%c%C+%c#2nd"), nil)
      T:neq(string.find(out.stderr[4], "%C+%c%C+%c%C+%c#2nd"), nil)
      T:neq(string.find(out.stderr[5], "%C+%c%C+%c%C+%c#3rd"), nil)
      T:neq(string.find(out.stderr[6], "%C+%c%C+%c%C+%c#3rd"), nil)
      T:neq(string.find(out.stderr[7], "%C+%c%C+%c%C+%c#4th"), nil)
      T:neq(string.find(out.stderr[8], "%C+%c%C+%c%C+%c#4th"), nil)
      cmd.rm{"-f", testdir .. "core-require-nodeps"}
      cmd.rm{"-f", testdir .. "core-require-last"}
    end
    require("test/core-require.lua")
    require_nodeps("test/core-require-nodeps.lua")
  end
T:done(N)

T:start"user-modules test/core-user-modules"
  do
    local user = function(policy)
      cfg{ "-f", policy}
      T:yes(stat.stat(testdir .. "core-user-modules"))
      cmd.rm { "-f", testdir .. "core-user-modules" }
    end
    user("test/core-user-modules/test.lua")
  end
T:done(N)

T:start"sub() test/core-sub.lua"
  do
    local sub = function(policy)
      cfg{"-f", policy}
      T:eq(cmd.rm{"-f", testdir.."FILE-1.0"}, 0)
    end
    sub("test/core-sub.lua")
  end
T:done(N)

T:start"each() test/core-each.lua"
  do
    local each = function(policy)
    cmd.mkdir{ "-p", testdir .. "core-each.xxx" }
    cmd.mkdir{ "-p", testdir .. "core-each.yyy" }
      cfg{ "-f", policy }
      T:no(os.is_dir(testdir .. "core-each.xxx"))
      T:no(os.is_dir(testdir .. "core-each.yyy"))
    end
    each"test/core-each.lua"
  end
T:done(N)

T:start"hostname.set (modules/hostname.lua)"
  do
    if path.bin"hostnamectl" then
      -- XXX duplicate copy from module.hostname
      local current_hostnames = function()
        local _, hostnamectl = cmd.hostnamectl{}
        local hostnames = {
          Pretty = false,
          Static = false,
          Transient = false
        }
        local _k, _v
        for ln = 1, #hostnamectl.stdout do
          for type, _ in next, hostnames do
            _k, _v = string.match(hostnamectl.stdout[ln], "^%s*(" .. type .. " hostname):%s([%g%s]*)$")
            if _k then
              -- New keys that starts with lower case characters.
              hostnames[string.lower(type)] = _v
            end
          end
        end
        return hostnames
      end
      local hostnames = current_hostnames()
      local before = {
        transient = hostnames.transient,
        pretty = hostnames.pretty,
        static = hostnames.static
      }
      local hostname = function(policy)
        cfg{ "-f", policy }
        local after = current_hostnames()
        T:eq(after.transient, "testing.configi.org")
        T:eq(after.pretty, "Testing Configi")
        T:eq(after.static, "static")
        for type, hostname in next, before do
          cmd.hostnamectl{ "--" .. type, "set-hostname", hostname }
        end
      end
      hostname"test/hostname_set.lua"
    else
      local _, out = cmd.hostname{}
      local before = out.stdout[1]
      local hostname = function(policy)
        cfg{ "-f", policy }
        _, out = cmd.hostname{}
        T:eq(out.stdout[1], "testing")
        cmd.hostname{ before }
        _, out = cmd.hostname{}
        T:eq(out.stdout[1], before)
      end
      hostname"test/hostname_set.lua"
    end
  end
T:done(N)

T:start"cron.present (modules/cron.lua) test/cron_present.lua"
  do
    local cron = function(policy)
      -- Restore root crontab file on OpenWRT
      if osfamily == "openwrt" then
        T:eq(cmd.touch{ "/etc/crontabs/root" }, 0)
      end
      cmd.crontab{ "-r" }
      cmd.crontab{ "-d" }
      cfg{ "-f", policy }
      local _, r = cmd.crontab{ "-l" }
      local t = table.filter(r.stdout, "^#[%C]+") -- Remove comments
      t[#t] = t[#t] .. "\n" -- Add trailing newline
      T:eq(crc(file.read_to_string("test/cron_present.out")), crc(table.concat(t, "\n")))
    end
    cron"test/cron_present.lua"
  end
T:done(N)

T:start"cron.absent (modules/cron.lua) test/cron_absent.lua"
  do
    local cron = function(policy)
      cfg{ "-f", policy }
      local _, r = cmd.crontab{ "-l" }
      local t = table.filter(r.stdout, "^#[%C]+") -- Remove comments
      t[#t] = t[#t] .. "\n" -- Add trailing newline
      T:eq(crc(file.read_to_string("test/cron_absent.out")), crc(table.concat(t, "\n")))
      cmd.crontab{ "-r" }
      cmd.crontab{ "-d" }
    end
    cron"test/cron_absent.lua"
  end
T:done(N)

T:start"template.render (modules/template.lua) test/template_render.lua"
  do
    local template = function(policy)
      local out = testdir .. "template_render_test.txt"
      cfg{ "-f", policy }
      T:eq(crc(file.read_to_string("test/template_render.txt")), crc(file.read_to_string(out)))
      os.remove(out)
    end
    template"test/template_render.lua"
  end
T:done(N)

T:start"edit.insert_line (modules/edit.lua) test/edit_insert.lua"
  do
    local edit = function(p1, p2)
      local out = testdir .. "edit_insert_test.txt"
      cfg{ "-f", "test/edit_insert.lua"}
      T:eq(crc(file.read_to_string("test/edit_insert.txt")), crc(file.read_to_string(out)))
      local _, ret = cfg{ "-mf", "test/edit_insert_inserts.lua"}
      T:neq(string.find(ret.stderr[1], "^[skip]*"), nil)
      T:eq(crc(file.read_to_string("test/edit_insert.txt")), crc(file.read_to_string(out)))
      os.remove(out)
    end
    edit("test/edit_insert.lua", "test/edit_insert_inserts.lua")
  end
T:done(N)

T:start"edit.insert_line_before (modules/edit.lua) test/edit_insert_line_before.lua"
  do
    local edit = function(policy)
      cfg{ "-f", policy }
      T:eq(crc(file.read_to_string("test/edit_insert_line_before.txt")),
        crc(file.read_to_string("test/tmp/edit_insert_line_test.txt")))
    end
    edit"test/edit_insert_line_before.lua"
  end
T:done(N)

T:start"edit.insert_line_after (modules/edit.lua) test/edit_insert_line_after.lua"
  do
    local edit = function(policy)
      cfg{ "-f", policy }
      T:eq(crc(file.read_to_string("test/edit_insert_line_after.txt")),
        crc(file.read_to_string("test/tmp/edit_insert_line_test.txt")))
      cmd.rm { "-f", testdir .. "edit_insert_line_test.txt" }
      cmd.rm { "-f", testdir .. "._configi_edit_insert_line_test.txt" }
    end
    edit"test/edit_insert_line_after.lua"
  end
T:done(N)

T:start"edit.remove_line (modules/edit.lua) test/edit_remove_line.lua"
  do
    local edit = function(policy)
      cfg{ "-f", policy }
      T:eq(crc(file.read_to_string("test/edit_remove_line.txt")),
        crc(file.read_to_string("test/tmp/edit_remove_line_test.txt")))
      cmd.rm { "-f", testdir .. "edit_remove_line_test.txt" }
    end
    edit"test/edit_remove_line.lua"
  end
T:done(N)

T:start"shell.command (modules/shell.lua)"
  do
    local shell = function(policy)
      cfg{ "-f", policy }
      T:yes(os.is_file(testdir .. "shell_command.txt"))
      os.remove(testdir .. "shell_command.txt")
    end
    shell"test/shell_command.lua"
  end
T:done(N)

T:start"shell.system (modules/shell.lua)"
  do
    local shell = function(policy)
      cfg{ "-f", policy }
      T:yes(os.is_file(testdir .. "shell_system.txt"))
      os.remove(testdir .. "shell_system.txt")
    end
    shell"test/shell_system.lua"
  end
T:done(N)

T:start"shell.popen (modules/shell.lua)"
  do
    local shell = function(p1, p2)
      cfg{ "-f", p1 }
      T:yes(os.is_file(testdir .. "shell_popen.txt"))
      os.remove(testdir .. "shell_popen.txt")
      cmd.touch{ testdir .. "The wizard quickly jinxed the gnomes before they vaporized" }
      T:eq(cfg{ "-f", p2 }, 0)
      os.remove(testdir .. "The wizard quickly jinxed the gnomes before they vaporized")
    end
    shell("test/shell_popen.lua", "test/shell_popen_expects.lua")
  end
T:done(N)

T:start"shell.popen3 (modules/shell.lua)"
  do
    local shell = function(p1, p2, p3)
      T:eq(cfg{ "-f", p1 }, 0)
      T:eq(cfg{ "-f", p2 }, 0)
      T:eq(cfg{ "-f", p3 }, 0)
    end
    shell("test/shell_popen3_stdin.lua", "test/shell_popen3_stdout.lua", "test/shell_popen3_stderr.lua")
  end
T:done(N)

T:start"user.present (modules/user.lua)"
  do
    local user = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    user"test/user_present.lua"
  end
T:done(N)

T:start"user.absent (modules/user.lua)"
  do
    local user = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    user"test/user_absent.lua"
  end
T:done(N)

if path.bin"apt-get" then
  T:start"apt.present (modules/apt.lua)"
    do
      local apt = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      apt"test/apt_present.lua"
    end
  T:done(N)
  T:start"apt.absent (modules/apt.lua)"
    do
      local apt = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      apt"test/apt_absent.lua"
    end
  T:done(N)
end

if path.bin"luarocks" then
  T:start"luarocks.install (modules/luarocks.lua)"
    do
      local luarocks = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
        T:eq(path.bin"luacheck", "/usr/bin/luacheck")
      end
      luarocks"test/luarocks_install.lua"
    end
  T:done(N)
  T:start"luarocks.remove (modules/luarocks.lua)"
    do
      local luarocks = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
        T:no(path.bin"luacheck")
      end
      luarocks"test/luarocks_remove.lua"
    end
  T:done(N)
end

if path.bin"yum" then
  T:start"yum.present (modules/yum.lua)"
    do
      local yum = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      yum"test/yum_present.lua"
    end
  T:done(N)
  T:start"yum.absent (modules/yum.lua)"
    do
      local yum = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      yum"test/yum_absent.lua"
    end
  T:done(N)
  T:start"yum.add_repo (modules/yum.lua)"
    do
      local yum = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
        local info = stat.stat("/etc/yum.repos.d/OpenResty.repo")
        T:neq(stat.S_ISREG(info.st_mode), 0)
        cmd.rm{ "-f", "/etc/yum.repos.d/OpenResty.repo" }
      end
      yum"test/yum_add_repo.lua"
    end
  T:done(N)
  T:start"yum.install groups (modules/yum.lua)"
    do
      local yum = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      yum"test/yum_groups.lua"
    end
  T:done(N)
end

if path.bin"systemctl" then
  T:start"systemd.started (modules/systemd.lua)"
    do
      local systemd = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      systemd"test/systemd_started.lua"
    end
  T:done(N)
  T:start"systemd.restart (modules/systemd.lua)"
    do
      local systemd = function(policy)
        local ok, pgrep = cmd.pgrep{ "nscd" }
        T:eq(ok, 0)
        local first = pgrep.stdout[1]
        cfg{ "-f", policy }
        ok, pgrep = cmd.pgrep{ "nscd" }
        T:eq(ok, 0)
        local second = pgrep.stdout[1]
        if first == second then
          ok = false
        end
        T:eq(ok, 0)
      end
      systemd"test/systemd_restart.lua"
    end
  T:done(N)
  T:start"systemd.reload (modules/systemd.lua)"
    do
      local systemd = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      systemd"test/systemd_reload.lua"
    end
  T:done(N)
  T:start"systemd.stopped (modules/systemd.lua)"
    do
      local systemd = function(policy)
        cfg{ "-f", policy }
        T:eq(cmd.pgrep{ "nscd" }, nil)
      end
      systemd"test/systemd_stopped.lua"
    end
  T:done(N)
  T:start"systemd.enabled (modules/systemd.lua)"
    do
      local systemd = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      systemd"test/systemd_enabled.lua"
    end
  T:done(N)
  T:start"systemd.disabled (modules/systemd.lua)"
    do
      local systemd = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      systemd"test/systemd_disabled.lua"
    end
  T:done(N)
end

if osfamily == "openwrt" then
  T:start"opkg.present (modules/opkg.lua)"
    do
      local opkg = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      opkg"test/opkg_present.lua"
    end
  T:done(N)
  T:start"opkg.absent (modules/opkg.lua)"
    do
      local opkg = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      opkg"test/opkg_absent.lua"
    end
  T:done(N)
  T:start"sysvinit.started (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        cfg{ "-f", policy }
        T:eq(cmd.pgrep{ "uhttpd" }, 0)
      end
      sysvinit"test/sysvinit_started.lua"
    end
  T:done(N)
  T:start"sysvinit.restart (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        local ok, cmd = cmd.pgrep{ "uhttpd" }
        local first = cmd.stdout[1]
        T:eq(ok, 0)
        cfg{ "-f", policy }
        ok, cmd = cmd.pgrep{ "uhttpd" }
        local second = cmd.stdout[1]
        T:eq(ok, 0)
        if first == second then
          ok = false
        end
        T:eq(ok, 0)
      end
      sysvinit"test/sysvinit_restart.lua"
    end
  T:done(N)
  T:start"sysvinit.reload (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      sysvinit"test/sysvinit_reload.lua"
    end
  T:done(N)
  T:start"sysvinit.stopped (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        cfg{ "-f", policy }
        T:no(cmd.pgrep{ "uhttpd" })
      end
      sysvinit"test/sysvinit_stopped.lua"
    end
  T:done(N)
  T:start"sysvinit.enabled (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      sysvinit"test/sysvinit_enabled.lua"
    end
  T:done(N)
  T:start"sysvinit.disabled (modules/sysvinit.lua)"
    do
      local sysvinit = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      sysvinit"test/sysvinit_disabled.lua"
    end
  T:done(N)
end

if osfamily == "gentoo" then
  T:start"portage.present (modules/portage.lua)"
    do
      local portage = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      portage"test/portage_present.lua"
    end
  T:done(N)
  T:start"portage.absent (modules/portage.lua)"
    do
      local portage = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      portage"test/portage_absent.lua"
    end
  T:done(N)
end

if osfamily == "alpine" then
  T:start"apk.present (modules/apk.lua)"
    do
      local apk = function(policy)
        T:eq(cfg{ "-v", "-f", policy }, 0)
      end
      apk"test/apk_present.lua"
    end
  T:done(N)
  T:start"apk.absent (modules/apk.lua)"
    do
      local apk = function(policy)
        T:eq(cfg{ "-v", "-f", policy }, 0)
      end
      apk"test/apk_absent.lua"
    end
  T:done(N)
end

if osfamily == "gentoo" or osfamily == "alpine" then
  T:start"openrc.started (modules/openrc.lua)"
    do
      local openrc = function(policy)
        cfg{ "-f", policy }
        T:eq(cmd.pgrep{ "rsync" }, 0)
      end
      openrc"test/openrc_started.lua"
    end
  T:done(N)
  T:start"openrc.restart (modules/openrc.lua)"
    do
      local openrc = function(policy)
        local ok, res = cmd.pgrep{ "rsync" }
        local first = res.stdout
        T:eq(ok, 0)
        cfg{ "-f", policy }
        ok, res = cmd.pgrep{ "rsync" }
        local second = res.stdout
        T:eq(ok, 0)
        if first == second then
          ok = false
        end
        T:eq(ok, 0)
      end
      openrc"test/openrc_restart.lua"
    end
  T:done(N)
  T:start"openrc.reload (modules/openrc.lua)"
    do
      local openrc = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      openrc"test/openrc_reload.lua"
    end
  T:done(N)
  T:start"openrc.stopped (modules/openrc.lua)"
    do
      local openrc = function(policy)
        cfg{ "-f", policy }
        T:eq(cmd.pgrep{ "rsync" }, nil)
      end
      openrc"test/openrc_stopped.lua"
    end
  T:done(N)
  T:start"openrc.add (modules/openrc.lua)"
    do
      local openrc = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      openrc"test/openrc_add.lua"
    end
  T:done(N)
  T:start"openrc.delete (modules/openrc.lua)"
    do
      local openrc = function(policy)
        T:eq(cfg{ "-f", policy }, 0)
      end
      openrc"test/openrc_del.lua"
    end
  T:done(N)
end

T:start"file.attributes (module/file.lua)"
  do
    local attributes = function(policy)
      local nobody = pwd.getpwnam("nobody")
      local nogroup = grp.getgrnam("nobody")
      cfg{ "-f", policy }
      local stat1 = stat.stat(testdir .. "file_attributes1")
      T:eq(stat1.st_uid, nobody.pw_uid)
      T:eq(stat1.st_gid, nogroup.gr_gid)
      T:eq(string.format("%o", stat1.st_mode), "100600")
      os.remove(testdir .. "file_attributes1")
      local stat2 = stat.stat(testdir .. "file_attributes2")
      T:eq(string.format("%o", stat2.st_mode), "100755")
      os.remove(testdir .. "file_attributes2")
      local stat3 = stat.stat(testdir .. "file_attributes3")
      T:eq(string.format("%o", stat3.st_mode), "100444")
      os.remove(testdir .. "file_attributes3")

    end
    attributes"test/file_attributes.lua"
  end
T:done(N)

T:start"file.link (module/file.lua)"
    do
        local link = function(policy)
            T:eq(cfg{ "-f", policy}, 0)
            local info = stat.lstat(testdir .. "file_link")
            T:neq(stat.S_ISLNK(info.st_mode), 0)
            T:eq(exec.system("rm -f " .. testdir .. "file_link"), 0)
        end
        link"test/file_link.lua"
    end
T:done(N)

T:start"file.hard (module/file.lua)"
    do
        local hard = function(policy)
            T:eq(cfg{ "-f", policy}, 0)
            local stat1 = stat.stat(testdir .. "file_hard_src")
            local stat2 = stat.stat(testdir .. "file_hard_link")
            T:eq(stat1.st_ino, stat2.st_ino)
            T:eq(exec.system("rm -f " .. testdir .. "file_hard_src"), 0)
            T:eq(exec.system("rm -f " .. testdir .. "file_hard_link"), 0)
        end
        hard"test/file_hard.lua"
    end
T:done(N)

T:start"file.directory (module/file.lua)"
    do
        local directory = function(policy)
            T:eq(cfg{ "-f", policy }, 0)
            local info = stat.stat(testdir .. "file_directory")
            T:neq(stat.S_ISDIR(info.st_mode), 0)
            T:yes(os.remove(testdir .. "file_directory"))
        end
        directory"test/file_directory.lua"
    end
T:done(N)

T:start"file.touch (module/file.lua)"
    do
        local touch = function(policy)
            T:eq(cfg{ "-f", policy }, 0)
            local info = stat.stat(testdir .. "file_touch")
            T:neq(stat.S_ISREG(info.st_mode), 0)
            T:yes(os.remove(testdir .. "file_touch"))
        end
        touch"test/file_touch.lua"
    end
T:done(N)

T:start"file.absent (module/file.lua)"
    do
        local absent = function(policy)
            T:eq(cfg{ "-f", policy }, 0)
            T:no(stat.stat(testdir .. "file_absent"))
        end
        absent"test/file_absent.lua"
    end
T:done(N)

T:start"file.copy (module/file.lua)"
    do
        local copy = function(policy)
            T:eq(cfg{ "-f", policy }, 0)
            local _, ls = cmd.ls{ "-1", testdir .. "file_copy_dest" }
            local stdout = table.concat(ls.stdout, "\n")
            T:yes(file.write_all(testdir .. "file_copy.tmp", stdout))
            T:eq(crc(file.read_to_string("test/file_copy.out")), crc(file.read_to_string(testdir .. "file_copy.tmp")))
            T:eq(cmd.rm{ "-rf", testdir .. "file_copy_dest" }, 0)
            T:eq(cmd.rm{ "-rf", testdir .. "file_copy_src" }, 0)
            T:eq(cmd.rm{ "-f", testdir .. "file_copy.tmp" }, 0)
        end
        copy"test/file_copy.lua"
    end
T:done(N)

T:start"authorized_keys.present (modules/authorized_keys.lua)"
  do
    local authorized_keys = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    authorized_keys"test/authorized_keys_present.lua"
  end
T:done(N)

T:start"authorized_keys.absent (modules/authorized_keys.lua)"
  do
    local authorized_keys = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    authorized_keys"test/authorized_keys_absent.lua"
  end
T:done(N)

T:start"unarchive.unpack (modules/unarchive.lua)"
  do
    local unarchive = function(policy)
      cfg{ "-f", "test/unarchive_unpack.lua" }
      T:yes(stat.stat(testdir .. "unarchive_unpack.lua"))
      cmd.rm{ "-f", testdir .. "unarchive_unpack.lua"}
    end
    unarchive"test/unarchive_unpack.lua"
  end
T:done(N)

if path.bin("git") then
  T:start"git.clone (modules/git.lua)"
    do
      local git = function(policy)
        cfg{ "-f", policy }
        T:yes(stat.stat(testdir .. "git/.git/config"))
      end
      git"test/git_clone.lua"
    end
  T:done(N)

  T:start"git.pull (modules/git.lua)"
    do
      local git = function(policy)
        local _, out = cfg{ "-vf", policy }
        out = table.concat(out.stderr, "\n")
        T:eq(string.find(out, "Already up-to-date.", 1, true), 70)
        cmd.rm{ "-rf", testdir .. "git" }
      end
      git"test/git_pull.lua"
    end
  T:done(N)
end

T:start"sha256.verify (modules/sha256.lua)"
  do
    local sha256 = function(policy)
      T:eq(cfg{ "-f", policy }, 0)
    end
    sha256"test/sha256_verify.lua"
  end
T:done(N)

T:start"iptables.append (modules/iptables.lua)"
  do
    local iptables = function(policy)
      cfg{ "-f", policy }
      local a = string.to_table"_ INPUT -s 6.6.6.6/32 -p tcp -m tcp --sport 31337 --dport 31337 -m comment --comment 'Configi' -j ACCEPT"
      a[1] = "-A"
      T:eq(cmd.iptables(a), 0)
      a[1] = "-D"
      T:eq(cmd.iptables(a), 0)
      cmd.iptables{ "-F" }
    end
    iptables"test/iptables_append.lua"
  end
T:done(N)

T:start"iptables.disable (modules/iptables.lua)"
  do
    local iptables = function(p1, p2)
      cfg{ "-f", p1 }
      local _, res = cmd.iptables{ "--list-rules" }
      T:eq(#res.stdout, 4)
      cfg{ "-f", p2 }
      _, res = cmd.iptables{ "--list-rules" }
      T:eq(#res.stdout, 3)
    end
    iptables("test/iptables_append.lua", "test/iptables_disable.lua")
  end
T:done(N)

T:start"make.install (modules/make.lua)"
  do
    local make = function(policy)
      cfg{ "-f", policy }
      T:eq(cmd.rm{ "-f", "test/tmp/root/bin/exe" }, 0)
      T:eq(cmd.rm{ "-r", "-f", "test/tmp/root" }, 0)
      T:eq(cmd.rm{ "-r", "-f", "test/tmp/make_install" }, 0)
    end
    make"test/make_install.lua"
  end
T:done(N)

T:start"process.running (modules/process.lua)"
  do
    local process = function(p1, p2)
      cfg{"-f", p1}
      T:yes(stat.stat("/tmp/RUNNING"))
      cfg{"-f", p2}
      T:yes(stat.stat("/tmp/NOT-RUNNING"))
      cmd.rm{"-f", "/tmp/RUNNING"}
      cmd.rm{"-f", "/tmp/NOT-RUNNING"}
    end
    process("test/process_running.lua", "test/process_not_running.lua")
  end
T:done(N)

T:start"process.signal (modules/process.lua)"
  do
    local process = function(p)
      cfg{"-f", p}
      T:yes(stat.stat("/tmp/NSCD-NOT-RUNNING"))
      cmd.rm{"-f", "/tmp/NSCD-NOT-RUNNING"}
    end
    process("test/process_signal.lua")
  end
T:done(N)

T:start"mount.mounted (modules/mount.lua)"
  do
    local mount = function(p)
      cfg{"-f", p}
      T:yes(file.find("/proc/mounts", "/configi-test-mount", true))
    end
    mount("test/mount_mounted.lua")
  end
T:done(N)


T:start"mount.opts (modules/mount.lua)"
  do
    local mount = function(p)
      cfg{"-f", p}
      local s = file.match("/proc/mounts", "tmpfs.+/configi%-test%-mount.*")
      T:eq(type(string.find(s, "nodev")), "number")
    end
    mount("test/mount_opts.lua")
  end
T:done(N)

T:start"mounts.unmounted (modules/mount.lua)"
  do
    local mount = function(p)
      cfg{"-f", p}
      T:no(file.find("/proc/mounts", "/configi-test-mount", true))
    end
    mount("test/mount_unmounted.lua")
  end
T:done(N)

--c.printf("\n  Summary: \n")
--c.printf("    %s Passed\n", N.successes)
--c.printf("    %s Failures\n\n", N.failures)



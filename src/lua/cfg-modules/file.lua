...
local table, path = lib.table, lib.path
...
local owner = function(F, P)
  ...
  table.insert_if(P.recurse, args, 2, "-R")
...
local mode = function(F, P)
  ...
  table.insert_if(P.recurse, args, 1, "-R")
...
function file.link(S)
  ...
  return function(P)
    ...
    table.insert_if(P.force, args, 2, "-f")
...
function file.hard(S)
  ...
  return function(P)
    ...
    table.insert_if(P.force, args, 1, "-f")
    if F.run(cmd.ln, args) then
      F.msg(P.path, M.report.repaired, true)
      return attrib(F, P, R)
    else
...
function file.directory(S)
  ...
  return function(P)
    ...
    if P.force then
      if P.backup then
        local dir, f = path.split(P.path)
        F.run(os.rename, P.path, dir .. "/._configi_" .. f)
      end

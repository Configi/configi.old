...
function user.present(S)
  ...
  return function(P)
    ...
    if path.bin"useradd" then
      ...
      table.insert_if(P.uid, args, 1, { "-u", P.uid })
      table.insert_if(P.gid, args, 1, { "-g", P.gid })
      table.insert_if(P.shell, args, 1, { "-s", P.shell })
      table.insert_if(P.home, args, 1, { "-d", P.home })
      table.insert_if(P.description, args, 1, { "-c", P.description })
      table.insert_if(P.expire_date, args, 1, { "-e", P.expire_date })
      table.insert_if(P.groups, args, 1, { "-G", P.groups })
...
function user.absent(S)
  ...
  return function(P)
    ...
    local ret
    local args = { P.login }
    if path.bin"userdel" then
      table.insert_if(P.remove, args, 1, "-r")

...
# Created: Mon 11 Mar 2019 07:23:21 AM UTC
#
#/ usage: promtail:create  --arg <> [ --comment <>] [ --verbose <>] [ --register <>] [ --requires <>] [ --loki <localhost>] [ --binary <./promtail>] 
...
rerun_options_parse() {
    ...
    do
        ...
        case "$OPT" in
            ...
            --loki) rerun_option_check $# $1; LOKI=$2 ; shift 2 ;;
            --binary) rerun_option_check $# $1; BINARY=$2 ; shift 2 ;;
            # help option
            -\?|--help)
                rerun_option_usage
                exit 2
    ...
    [[ -z "$LOKI" ]] && LOKI="$(rerun_property_get $RERUN_MODULE_DIR/options/loki DEFAULT)"
    [[ -z "$BINARY" ]] && BINARY="$(rerun_property_get $RERUN_MODULE_DIR/options/binary DEFAULT)"
